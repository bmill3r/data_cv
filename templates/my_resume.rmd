...
.skill-category {
  ...
  margin: 0 !important;
  padding: 0 !important;
  height: 0.8em !important; /* Reduced from 1.2em */
  line-height: 0.8 !important; /* Added to reduce vertical space */
}
...
.skill-item {
  padding-left: 1em;
  display: block;
  margin-top: -0.20em !important; /* Increased negative margin to pull up skill items further */
...
if (!is.null(params$custom_image_path) && file.exists(params$custom_image_path)) {
  ...
  # Fall back to network logo if no custom image
  build_network_logo_custom(CV$entries_data, margin_top = "-10px", margin_bottom = "0px", width = "100%", height = "200px")
}
```
